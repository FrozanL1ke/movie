...
const getNav = () => {
  ...
  if (navItems) {
    ...
    const tamesmarks = getCurrentWeek().tamesmarks;
    navItems.forEach((item, index) => {
      ...
      item.setAttribute("data-tamestamp", tamesmarks[index]);
      ...
      if (currentWeek[index] === currentDay) {
        ...
        TimeMarkToday = item.getAttribute("data-tamestamp");
        isCurrentDay = true;
      }
      ...
      item.addEventListener("click", () => {
        ...
        navItems.forEach(i => i.classList.remove("page-nav__day_chosen"));
  ...
  function getCurrentWeek() {
    ...
    return { week, daysOfTheWeek, tamesmarks };
  }
};
  ...
  const getHall = (hall, currentHalls, filmName) => {
    ...
    return `
      ...
      ${currentHalls.map((seance, index) => {
        ...
        const timestamp = Number(TimeMarkToday) + seance.seance_start * 60;
